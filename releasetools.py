...
def GetOmniProps(info):
  ...
  info.script.AppendExtra('ifelse(sha1_check(read_file("/system/addon.d/omnirom.prop")) == "",')
  info.script.AppendExtra('     (ui_print("/tmp/omnirom.prop not found");),')
  info.script.AppendExtra('     (ui_print("/system/addon.d/omnirom.prop found");run_program("/sbin/busybox", "cp", "/system/addon.d/omnirom.prop", "/tmp/omnirom.prop");));')
